    ...
    app.get('/data/findcontract/:melicode', mypassport.ensureAuthenticated, function (req, res) {
        var callback = function (err, rows) {
            if (err) {
                ...
                for (var row in rows) {
                    formatdata(rows[row]);
                    console.log('t*********est', rows[row]);
                }
                res.json(rows);
            }
        };
        db.all('SELECT * from requests where ((user=' + req.user.id  + ' OR owner=' + req.user.id + ') AND applicant=? AND requesttype="contract")', req.params.melicode, callback);
    });
};
